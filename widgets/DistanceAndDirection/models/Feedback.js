...
define([
  ...
  var w = dojoDeclare([esriDraw, dojoStateful], {
    ...
    initGeometryService: function () {
      ...
      this._fetchPortalGeomSrvcUrl().then(dojoLang.hitch(this, function (geomsrvcUrl) {
        // If geometry service is not available in portal helper services then use the app config geometry service
        if (dojoLang.trim(geomsrvcUrl) === "") {
          geomsrvcUrl = gs;
        }
        //initiate geometry service
        this.geomService = new EsriGeometryService(geomsrvcUrl);
      }));
    ...
    /**
     * This will fetch geometry service from portal helpservices 
     **/
    _fetchPortalGeomSrvcUrl: function () {
      var def = new dojoDeferred();
      //check whether portal url is available then try to fetch helper services
      if (this.appConfig.portalUrl && dojoLang.trim(this.appConfig.portalUrl) !== "") {
        //get portal info to fetch geometry service Url
        portalUtils.getPortalSelfInfo(this.appConfig.portalUrl).then(dojoLang.hitch(
          this,
          function (portalInfo) {
            // get helper-services from portal object
            var helperServices = portalInfo && portalInfo.helperServices;
            var geomsrvcUrl = "";
            if (helperServices && helperServices.geometry && helperServices.geometry.url &&
              helperServices.geometry.url !== "") {
              geomsrvcUrl = helperServices.geometry.url;
            }
            def.resolve(geomsrvcUrl);
          }), dojoLang.hitch(this, function () {
            def.resolve("");
          }));
      } else {
